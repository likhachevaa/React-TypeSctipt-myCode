...
import { createPaymentThunkAction } from "store/Scan/Scan.actions";
import { showErrorSnackbar, showSuccessSnackbar } from "store/Notifications";
import { useReculatePaymentMutation } from "store/Scan/Scan.service";
...
export const PayModalScan = ({ index }: IProps) => {
  const dispatch = useDispatch();
  const { t } = useTranslation(["publicOffer", "auth", "scan"]);
  ...
  const handlePromo = useCallback(
    async (data: IPayForm) => {
      ...
      try {
        ...
        setValue("promocode", " ");
        dispatch(showSuccessSnackbar(t`scan:promocodeSuccess`));
      } catch (e) {
        dispatch(showErrorSnackbar(t`scan:promocodeFalied`));
      }
    },
        ...
        <FormProvider {...methods}>
          <Box component="form">
            <CardContent sx={styles.content}>
              ...
              <Box>
                <Typography> {t`scan:numberCards`} </Typography>
                <Box
                  sx={{
                    display: "flex",
                    paddingTop: " 14px",
              ...
              >
                <Typography sx={styles.tariffs}>
                  {index === 1
                    ? t`scan:productAnalysis`
                    : t`scan:brandAnalysis`}
                </Typography>
                <Typography sx={styles.infoText}>
                  {index === 1
                    ? t`scan:priceCardProduct`
                    : t`scan:priceCardBrand`}
                </Typography>
              </Box>
